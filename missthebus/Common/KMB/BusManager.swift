...
class BusManager{
    ...
    static func saveAllStops(_ stops: [Stop]){
        do {
            ...
            Storage.save(suiteName: Configs.SuiteName.AppGroup, Configs.Storage.KEY_STOPS, data)
        } catch {
            print("Unable to Encode Note (\(error))")
        }
    }
    ...
    static func getAllStops() -> [Stop]?{
        if let data = Storage.getObject(suiteName: Configs.SuiteName.AppGroup, Configs.Storage.KEY_STOPS){
            do {
                let decoder = JSONDecoder()
